...
from ratelimitex.decorators import adaptive_rate_limited, burst_rate_limited, rate_limited
...
async def test_rate_limited_decorator_prevention():
    """Test that the rate_limited decorator prevents requests when limit is exceeded."""
    request_count = 0
...
async def test_decorator_with_custom_config():
    ...
    config = RateLimitConfig(max_requests=2, time_window=1, strategy=RateLimitStrategy.STRICT)

    @rate_limited(
        max_requests=config.max_requests, time_window=config.time_window, strategy=config.strategy
    )
    async def test_function():
        nonlocal request_count
        request_count += 1
